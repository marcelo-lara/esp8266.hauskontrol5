...
Fan::Fan(bool _single, int _pin1,int _pin2,int _pin3,int _pin4, int _defaultSpeed) : Device(Device::DevType_e::Fan, "fan") {
    single = _single;
    out[0] = _pin1;
    out[1] = _pin2;
    out[2] = _pin3;
...
void Fan::render_single(){
    for (int i = 0; i < this->max_speed; i++) {
        pinMode(this->out[i], OUTPUT);
        digitalWrite(this->out[i], ((this->speed - 1 == i) ? (this->inv_out?0:1) : (this->inv_out?1:0)) );
    }
};
